...
int amp_cnt = 0;
int amp_thresh = 5;            //loop count for consecutive amperage alarms to prevent false positives
  ...
  if (pwm_oe) {
    ...
    if (!move_demo && !move_funplay) {
      if (ps2x.Button(PSB_START)) {
        ...
        if (ps2_select == 1) {
          if (mpu_active) {
            Serial.println(F("Disabled MPU"));
            mpu_active = 0;
          } else {
            Serial.println(F("Enabled MPU"));
            mpu_active = 1;
          }
          ...
          if (uss_active) {
            Serial.println(F("Disabled USS"));
            uss_active = 0;
          } else {
            Serial.println(F("Enabled USS"));
            uss_active = 1;
          }
